...
if arch == capstone.CS_ARCH_ARM:
    if cs_operand.type == capstone.arm_const.ARM_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.arm_const.ARM_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.arm_const.ARM_OP_IMM: 
        typ = OperandType.immediate
    elif cs_operand.type == capstone.arm_const.ARM_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.arm_const.ARM_OP_FP: # Floating-Point operand 
        typ = OperandType.float_point
    elif cs_operand.type == capstone.arm_const.ARM_OP_CIMM: # C-Immediate (coprocessor registers)
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm_const.ARM_OP_PIMM: # P-Immediate (coprocessor registers) 
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm_const.ARM_OP_SETEND: # operand for SETEND instruction
        typ = OperandType.arm_setend
    elif cs_operand.type == capstone.arm_const.ARM_OP_SYSREG: # MSR/MRS special register operand
        typ = OperandType.coprocessor
     
elif arch == capstone.CS_ARCH_ARM64:
    if cs_operand.type == capstone.arm64_const.ARM64_OP_INVALID: 
        typ = OperandType.unknown
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_IMM: 
        typ = OperandType.immediate
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_MEM: 
        typ = OperandType.memory
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_FP: # Floating-Point operand
        typ = OperandType.float_point
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_CIMM: # C-Immediate (coprocessor registers)
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_REG_MRS: # MRS register operand
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_REG_MSR: # MSR register operand
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_PSTATE: # PState operand
        # Maybe change this for it's own operand type
        typ = OperandType.register 
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_SYS: # SYS operand for IC/DC/AT/TLBI instructions 
        typ = OperandType.arm_memory_management
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_SVCR: # SVCR operand for MSR SVCR instructions 
        typ = OperandType.coprocessor
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_PREFETCH: # Prefetch operand (PRFM)
        typ = OperandType.arm_memory_management
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_BARRIER: # Memory barrier operand (ISB/DMB/DSB instructions)
        typ = OperandType.arm_memory_management
    elif cs_operand.type == capstone.arm64_const.ARM64_OP_SME_INDEX: # SME instruction operand with with index 
        typ = OperandType.arm_sme

elif arch == capstone.CS_ARCH_MIPS:
    if cs_operand.type == capstone.mips_const.MIPS_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.mips_const.MIPS_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.mips_const.MIPS_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.mips_const.MIPS_OP_MEM:  
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_X86:
    if cs_operand.type == capstone.x86_const.X86_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.x86_const.X86_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.x86_const.X86_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.x86_const.X86_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_PPC:
    if cs_operand.type == capstone.ppc_const.PPC_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.ppc_const.PPC_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.ppc_const.PPC_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.ppc_const.PPC_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.ppc_const.PPC_OP_CRX: # Condition Register field
        typ = OperandType.register 

elif arch == capstone.CS_ARCH_SPARC:
    if cs_operand.type == capstone.sparc_const.SPARC_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.sparc_const.SPARC_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.sparc_const.SPARC_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.sparc_const.SPARC_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_SYSZ:
    if cs_operand.type == capstone.sysz_const.SYSZ_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.sysz_const.SYSZ_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.sysz_const.SYSZ_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.sysz_const.SYSZ_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.sysz_const.SYSZ_OP_ACREG: # Access register operand
        typ = OperandType.register 

elif arch == capstone.CS_ARCH_XCORE:
    if cs_operand.type == capstone.xcore_const.XCORE_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.xcore_const.XCORE_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.xcore_const.XCORE_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.xcore_const.XCORE_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_M68K:
    if cs_operand.type == capstone.m68k_const.M68K_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.m68k_const.M68K_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.m68k_const.M68K_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.m68k_const.M68K_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.m68k_const.M68K_OP_FP_SINGLE: # single precision Floating-Point operand
        typ = OperandType.float_point
    elif cs_operand.type == capstone.m68k_const.M68K_OP_FP_DOUBLE: # double precision Floating-Point operand
        typ = OperandType.float_point
    elif cs_operand.type == capstone.m68k_const.M68K_OP_REG_BITS: # Register bits move 
        typ = OperandType.register 
    elif cs_operand.type == capstone.m68k_const.M68K_OP_REG_PAIR: # Register pair in the same op (upper 4 bits for first reg, lower for second) 
        typ = OperandType.register 
    elif cs_operand.type == capstone.m68k_const.M68K_OP_BR_DISP: # Branch displacement
        typ = OperandType.memory
    
elif arch == capstone.CS_ARCH_TMS320C64X:
    if cs_operand.type == capstone.tms320c64x_const.TMS320C64X_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.tms320c64x_const.TMS320C64X_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.tms320c64x_const.TMS320C64X_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.tms320c64x_const.TMS320C64X_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.tms320c64x_const.TMS320C64X_OP_REGPAIR: # Register pair for double word ops
        typ = OperandType.register 
    
elif arch == capstone.CS_ARCH_M680X:
    if cs_operand.type == capstone.m680x_const.M680X_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.m680x_const.M680X_OP_REGISTER:
        typ = OperandType.register 
    elif cs_operand.type == capstone.m680x_const.M680X_OP_IMMEDIATE:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.m680x_const.M680X_OP_INDEXED: # Indexed addressing operand
        typ = OperandType.memory
    elif cs_operand.type == capstone.m680x_const.M680X_OP_EXTENDED: # Extended addressing operand
        typ = OperandType.memory
    elif cs_operand.type == capstone.m680x_const.M680X_OP_DIRECT: # Direct addressing operand
        typ = OperandType.memory
    elif cs_operand.type == capstone.m680x_const.M680X_OP_RELATIVE: # Relative addressing operand 
        typ = OperandType.memory
    elif cs_operand.type == capstone.m680x_const.M680X_OP_CONSTANT: # constant operand (Displayed as number only) 
        typ = OperandType.immediate

elif arch == capstone.CS_ARCH_MOS65XX:
    if cs_operand.type == capstone.mos65xx_const.MOS65XX_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.mos65xx_const.MOS65XX_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.mos65xx_const.MOS65XX_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.mos65xx_const.MOS65XX_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_WASM:
    if cs_operand.type == capstone.wasm_const.WASM_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.wasm_const.WASM_OP_NONE: # Not documented by capstone 
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_INT7: # Not documented by capstone
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_VARUINT32: # Not documented by capstone
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_VARUINT64: # Not documented by capstone
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_UINT32: # Not documented by capstone
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_UINT64: # Not documented by capstone
        pass
    elif cs_operand.type == capstone.wasm_const.WASM_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.wasm_const.WASM_OP_BRTABLE: # Not documented by capstone
        pass

elif arch == capstone.CS_ARCH_BPF:
    if cs_operand.type == capstone.bpf_const.BPF_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.bpf_const.BPF_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.bpf_const.BPF_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.bpf_const.BPF_OP_OFF: # Offset
        pass
    elif cs_operand.type == capstone.bpf_const.BPF_OP_MEM:
        typ = OperandType.memory
    elif cs_operand.type == capstone.bpf_const.BPF_OP_MMEM: # M[k] in ccapstone.bpf_const.BPF
        pass
    elif cs_operand.type == capstone.bpf_const.BPF_OP_MSH: # corresponds to ccapstone.bpf_const.BPF's capstone.bpf_const.BPF_MSH mode
        pass
    elif cs_operand.type == capstone.bpf_const.BPF_OP_EXT: # ccapstone.bpf_const.BPF's extension (not ecapstone.bpf_const.BPF) 
        pass

elif arch == capstone.CS_ARCH_RISCV:
    if cs_operand.type == capstone.riscv_const.RISCV_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.riscv_const.RISCV_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.riscv_const.RISCV_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.riscv_const.RISCV_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_SH:
    if cs_operand.type == capstone.sh_const.SH_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.sh_const.SH_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.sh_const.SH_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.sh_const.SH_OP_MEM:
        typ = OperandType.memory

elif arch == capstone.CS_ARCH_TRICORE:
    if cs_operand.type == capstone.tricore_const.TRICORE_OP_INVALID:
        typ = OperandType.unknown
    elif cs_operand.type == capstone.tricore_const.TRICORE_OP_REG:
        typ = OperandType.register 
    elif cs_operand.type == capstone.tricore_const.TRICORE_OP_IMM:
        typ = OperandType.immediate
    elif cs_operand.type == capstone.tricore_const.TRICORE_OP_MEM:
        typ = OperandType.memory
