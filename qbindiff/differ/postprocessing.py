...
def convert_matching(adds1: AddrIndex, adds2: AddrIndex, matching: BeliefMatching) -> Matching:
    ...
    """
    match1, match2 = map(list, zip(*filter(lambda x: x[1] is not None, matching)))
    madds1 = adds1[match1]
    madds2 = adds2[match2]
...
def get_candidates(address: int, program1: Program, program2: Program, matching: Matching) -> Optional[Set[Addr]]:
    ...
    if program1[address].parents:
        candidates.update(reduce(set.intersection, [program2[f_p2].children for f_p2 in
                                                    map(matching.get, program1[address].parents) if f_p2], set()))

    if program1[address].children:
        candidates.update(reduce(set.intersection, [program2[f_p2].parents for f_p2 in
                                                    map(matching.get, program1[address].children) if f_p2], set()))
    return candidates
